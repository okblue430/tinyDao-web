    ...
    const send = async (event) => {
        ...
        if(isNaN(amount)) {
            ...
            try {
                ...
                const res = await donation.donateEth({value: eth_amount})
                console.log({res})
                setAmount(0)
            } catch (error) {
    ...
    return (
        <div className="w-full max-w-md border-gray-50 border-t hover:shadow mx-auto mt-10 border-radius-6 mb-4">
            <form className="bg-white shadow-md rounded px-8 pt-6 pb-8" onSubmit={send}>
                <p className='text-black text-3xl text-bold mb-3'>Donation <span className='text-sm'>(only eth)</span></p>
                <div className="flex items-center justify-between">
                    <div className='text-right'>
                        <input className="mt-2 shadow appearance-none border rounded py-2 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" 
                            placeholder="amount"
                            type="text" 
                            value={amount}
